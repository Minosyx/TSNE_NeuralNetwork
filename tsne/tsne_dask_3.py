...
def x2p(X=np.array([]), tol=1e-5, perplexity=30.0):
    ...
    X_tmp = da.from_array(X, chunks=('auto'))
    
    sum_X = da.sum(da.square(X_tmp), 1)
    D = da.add(da.add(-2 * da.dot(X_tmp, X_tmp.T), sum_X).T, sum_X) # || xi - xj || ^ 2
    P = da.zeros((n, n), chunks=('auto'))
    beta = da.ones((n, 1), chunks=('auto'))
    ...
    for i in range(n):
        ...
        Di = Di.compute()
        beta_i = beta[i] # bez compute bedzie stalo w miejscu
        ...
        Pi = P[i]
        Pi[np.concatenate((np.r_[0:i], np.r_[i+1:n]))] = thisP
        P[i] = Pi

    # Return final P-matrix
    print("Mean value of sigma: %f" % da.mean(da.sqrt(1 / beta)).compute())
    return P.compute()
