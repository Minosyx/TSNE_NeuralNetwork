...
def kde2d(x, y, n=256, limits=None):
    ...
    (binned, xedges, yedges) = histogram2d(
        x, y, bins=n, range=((xmin, xmax), (ymin, ymax))
    )
    ...
    ψ20 = ψ(2, 0, ts)
    ψ11 = ψ(1, 1, ts)
    tx1 = (ψ02 ** (3 / 4) / (4 * π * N * ψ20 ** (3 / 4) * (ψ11 + sqrt(ψ02 * ψ20)))) ** (
        1 / 3
    )
    tx2 = (ψ20 ** (3 / 4) / (4 * π * N * ψ02 ** (3 / 4) * (ψ11 + sqrt(ψ02 * ψ20)))) ** (
        1 / 3
    )
    ...
    smoothed = transformed * outer(
        exp(-(π**2) * k2 * tx2 / 2), exp(-(π**2) * k2 * tx1 / 2)
    )
