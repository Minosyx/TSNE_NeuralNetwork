...
def x2p(X=np.array([]), tol=1e-5, perplexity=30.0):
    ...
    for i in range(n):
        ...
        betamax = np.inf
        Di = D[
            i, np.concatenate((np.r_[0:i], np.r_[i + 1 : n]))
        ]  # slice of D excluding i-th row
...
def pca(X=np.array([]), no_dims=50):
    ...
    (n, d) = X.shape  # shape of X
    X = X - np.tile(np.mean(X, 0), (n, 1))
    (l, M) = np.linalg.eig(
        np.dot(X.T, X)
    )  # calculate eigenvalues and eigenvectors of covariance matrix
    Y = np.dot(X, M[:, 0:no_dims])  # calculate eigenvectors for eigenvalues
    return Y
...
def tsne(
    ...
    if initial_dims > 0:  # if initial_dims > 0, run PCA first
        print(f"Initializing with {initial_dims} dimensions")
        X = pca(X, initial_dims).real
    (n, d) = X.shape
    initial_momentum = 0.5  # alpha(t) in gradient descent
    final_momentum = 0.8
    eta = 500
    min_gain = 0.01
    Y = np.random.randn(n, no_dims)  # init of Y with random Gaussian values
    dY = np.zeros((n, no_dims))
    iY = np.zeros((n, no_dims))
    gains = np.ones((n, no_dims))

    # Compute P-values
    P = x2p(X, 1e-5, perplexity)  # P find
    P = P + np.transpose(P)
